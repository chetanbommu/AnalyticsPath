data1 = read.csv("Files:Data/titanic.csv", na.strings=c(""," ","NA","?","  "))
...
?confusionMatrix
confusionMatrix(data=as.factor(preds_train$preds_Class),reference =trainData$survived, mode="prec_recall")
...
confusionMatrix(data=as.factor(preds$preds_Class),reference =testData$survived, mode="prec_recall")
...
library(adabag)
bagModel = bagging(survived~.,data = trainData, mfinal = 50,control=c(maxdepth = 10))
preds = predict(bagModel,trainData,type='response')
table(trainData$survived, preds$class,dnn=c('acts','preds'))
confusionMatrix(data=as.factor(preds$class),reference =trainData$survived, mode="prec_recall")
